...
<head>
    ...
    <title>Đơn hàng của tôi - Shipper</title>
...
<main class="main-content py-5">
    <div class="container">
        <c:set var="pageTitle" value="Đơn hàng được gán cho tôi" />
        <jsp:include page="/WEB-INF/views/shared/page-title.jsp" />

        <%-- include shipper navigation to match dashboard layout --%>
        <jsp:include page="/WEB-INF/views/shared/shipper_nav.jsp" />

        <div class="checkout-panel">
            <h4 class="section-title">
                <i class="fas fa-truck"></i>
                Đơn hàng được gán cho tôi
            </h4>
            ...
            <c:choose>
                <c:when test="${not empty orders}">
                    <div class="checkout-panel">
                        ...
                        <div class="accordion" id="shipperOrdersAccordion">
                                ...
                                <div class="accordion-item order-card" data-status="${order.status}">
                                    <h2 class="accordion-header" id="heading-${order.id}">
                                        <button class="accordion-button ${status.first ? '' : 'collapsed'}" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-${order.id}">
                                            <span class="order-accordion-header" style="display:grid; grid-template-columns: 1fr 140px 220px 150px; gap:.75rem; align-items:center; width:100%;">
                                                <span class="order-id">Đơn hàng #${order.id}</span>
                                                <span class="order-date"><i class="fas fa-clock me-1"></i><fmt:formatDate value="${order.legacyCreatedAt}" pattern="dd/MM/yyyy HH:mm" /></span>
                                                ...
                                                <span class="order-total" style="text-align:right; font-weight:700; color:var(--accent-pink);">
                                    ...
                                    <div id="collapse-${order.id}" class="accordion-collapse collapse ${status.first ? 'show' : ''}" data-bs-parent="#shipperOrdersAccordion">
                                        <div class="accordion-body">
                                            <div class="row g-3">
                                                ...
                                                <div class="col-lg-6">
                                                    ...
                                                    <div class="d-flex gap-2 justify-content-lg-end" style="margin-top: .75rem;">
                                                        <c:choose>
                                                            <c:when test="${order.status == 'Chờ lấy hàng'}">
                                                                <form action="<c:url value='/shipper/orders/update-status'/>" method="post">
                                                                    <input type="hidden" name="orderId" value="${order.id}">
                                                                    <input type="hidden" name="status" value="Đang giao">
                                                                    <button type="submit" class="btn btn-primary">
                                                                        <i class="fas fa-box me-1"></i>Đã lấy hàng
                                                                    </button>
                                                                </form>
                                                            </c:when>
                                                            <c:when test="${order.status == 'Đang giao'}">
                                                                <form action="<c:url value='/shipper/orders/update-status'/>" method="post" class="d-inline-block mb-1">
                                                                    <input type="hidden" name="orderId" value="${order.id}">
                                                                ...
                                                                <form action="<c:url value='/shipper/orders/update-status'/>" method="post" class="d-inline-block">
                                                                    <input type="hidden" name="orderId" value="${order.id}">
                                                            ...
                                                            <c:otherwise>
                                                                <p class="text-muted mb-0">Trạng thái: <strong>${order.status}</strong></p>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </div>
                                            ...
                                            <ul class="list-group list-group-flush">
                                                <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                                    <span>Tiền hàng</span>
                                                    <span><fmt:formatNumber value='${productSubtotal}' type="currency" currencySymbol="" maxFractionDigits="0"/> VNĐ</span>
                                                </li>
                                                <c:if test='${not empty order.shippingCarrier}'>
                                                    <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                                        <span>Phí vận chuyển (<c:out value='${order.shippingCarrier.name}'/>)</span>
                                                        <span><fmt:formatNumber value='${order.shippingCarrier.shippingFee}' type="currency" currencySymbol="" maxFractionDigits="0"/> VNĐ</span>
                                                    </li>
                                                </c:if>
                                                ...
                                                <c:if test='${currentDiscountAmount > 0}'>
                                                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 text-success">
                                                        ...
                                                        <span>- <fmt:formatNumber value='${currentDiscountAmount}' type="currency" currencySymbol="" maxFractionDigits="0"/> VNĐ</span>
                                                    </li>
                                                </c:if>
                                                <li class="list-group-item d-flex justify-content-between align-items-center px-0 fw-bold">
                                                    <span>Tổng cộng</span>
                                                    <span><fmt:formatNumber value='${order.totalAmount - currentDiscountAmount}' type="currency" currencySymbol="" maxFractionDigits="0"/> VNĐ</span>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                ...
                <c:otherwise>
                    <div class="text-center border rounded orders-empty">
                        <i class="fas fa-box-open fa-3x text-muted mb-3"></i>
                        <h2>Bạn chưa có đơn hàng nào.</h2>
                        <p class="lead">Vào trang "Tìm đơn hàng mới" để bắt đầu nhận đơn.</p>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
