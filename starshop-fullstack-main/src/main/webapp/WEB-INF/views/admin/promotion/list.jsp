    ...
    <div class="container">
        ...
        <div class="checkout-panel">
            <div class="section-title">
                ...
                <a href="<c:url value='/admin/promotion/add'/>" class="btn btn-themed">
                    <i class="fas fa-plus"></i> Thêm mới
                </a>
            </div>

            <%-- Search Form --%>
            <form action="<c:url value='/admin/promotion'/>" method="get" class="mb-4">
                <div class="input-group">
                    <input type="search" name="keyword" class="form-control" placeholder="Tìm kiếm theo mã, mô tả, tên shop..." value="${keyword}" aria-label="Tìm kiếm khuyến mãi">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i></button>
                    <c:if test="${not empty keyword}">
                        <a href="<c:url value='/admin/promotion'/>" class="btn btn-outline-secondary"><i class="fas fa-times"></i></a>
                    </c:if>
                </div>
            </form>
            ...
            <c:choose>
                <c:when test="${not empty promotions}">
                    <ul class="item-list">
                        ...
                        <c:forEach var="promotion" items="${promotions}">
                            <c:set var="isExpired" value="${promotion.endDate.isBefore(today)}"/>
                            <c:set var="notStarted" value="${promotion.startDate.isAfter(today)}"/>
                            <c:set var="isActive" value="${promotion.active && !isExpired && !notStarted}" />

                            <li class="item-list-item">
                                <div class="d-flex align-items-center flex-grow-1 min-width-0">
                                    <div class="item-icon ${isActive ? 'icon-active' : 'icon-inactive'}">
                                        <i class="fas fa-gift"></i>
                                    </div>
                                    <div class="item-details">
                                        <div class="item-title">${promotion.code}</div>
                                        <div class="item-sub">
                                            <c:if test="${not empty promotion.shop.name}">
                                                Shop: ${promotion.shop.name} |
                                            </c:if>
                                            Giảm ${promotion.discountPercentage}%
                                            <c:if test="${not empty promotion.maxDiscountAmount}">
                                                (Tối đa <fmt:formatNumber value="${promotion.maxDiscountAmount}" type="currency" currencyCode="VND" minFractionDigits="0"/>)
                                            </c:if>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex align-items-center item-actions">
                                    <div class="text-center me-4 d-none d-md-block">
                                        ...
                                        <div class="text-muted small mt-1">ID: ${promotion.id}</div>
                                    </div>

                                    <a href="<c:url value='/admin/promotion/edit/${promotion.id}'/>" class="action-btn action-edit" title="Sửa">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <a href="<c:url value='/admin/promotion/toggle-status/${promotion.id}'/>"
                                       class="action-btn action-toggle"
                                       title="${promotion.active ? 'Tạm dừng' : 'Kích hoạt'}"
                                       onclick="return confirm('Bạn có chắc muốn ${promotion.active ? 'tạm dừng' : 'kích hoạt'} mã khuyến mãi này?')">
                                        <i class="fas fa-${promotion.active ? 'pause' : 'play'}"></i>
                                    </a>
                                    <a href="<c:url value='/admin/promotion/delete/${promotion.id}'/>" class="action-btn action-delete" title="Xóa" onclick="return confirm('Bạn có chắc chắn muốn xóa mã khuyến mãi này?');">
                ...
                <c:otherwise>
                     <div class="text-center p-5 border rounded bg-light">
                        ...
                        <h5 class="mb-2">Không tìm thấy khuyến mãi nào</h5>
                        <p class="text-muted mb-3">
                            <c:if test="${not empty keyword}">
                                Không có khuyến mãi nào khớp với từ khóa "<c:out value="${keyword}"/>".
                            </c:if>
                            <c:if test="${empty keyword}">
                                Hiện chưa có khuyến mãi nào trong hệ thống.
                            </c:if>
                        </p>
                        <a href="<c:url value='/admin/promotion'/>" class="btn btn-primary">Xem tất cả khuyến mãi</a>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
