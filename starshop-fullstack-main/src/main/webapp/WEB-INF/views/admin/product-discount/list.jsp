...
<head>
    ...
    <title>Quản Lý Giảm Giá Sản Phẩm - Admin</title>
    ...
    <style>
        ...
        .item-icon {
            width: 84px;
            height: 84px;
            border-radius: 10px; /* Matched from product/list.jsp */
            display: flex;
            align-items: center;
        ...
        .item-icon i { font-size: 2rem; line-height: 1; color: #9ca3af; }
        ...
        .badge-status.status-expired { background: linear-gradient(90deg, #FECACA, #F87171); color: #7F1D1D; }
        .badge-status.status-pending { background: linear-gradient(90deg, #FDE68A, #FBBF24); color: #78350F; }
        ...
        .action-toggle { background: linear-gradient(90deg, #FBBF24, #F59E0B); } /* amber */
    ...
    <div class="container">
        ...
        <div class="admin-top-sticky mb-3">
            ...
            <ul class="nav nav-pills profile-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="<c:url value='/admin/product-discount'/>">
        ...
        <div class="checkout-panel">
            <div class="section-title">
                <div class="title-content">
                    ...
                    <span>Quản Lý Giảm Giá Sản Phẩm</span>
                </div>
                <a href="<c:url value='/admin/product-discount/add'/>" class="btn btn-themed">
                    <i class="fas fa-plus"></i> Thêm mới
                </a>
            </div>

            <%-- Search Form --%>
            <form action="<c:url value='/admin/product-discount'/>" method="get" class="mb-4">
                <div class="input-group">
                    <input type="search" name="keyword" class="form-control" placeholder="Tìm kiếm theo tên sản phẩm, cửa hàng..." value="${keyword}" aria-label="Tìm kiếm giảm giá">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i></button>
                    <c:if test="${not empty keyword}">
                        <a href="<c:url value='/admin/product-discount'/>" class="btn btn-outline-secondary"><i class="fas fa-times"></i></a>
                    </c:if>
                </div>
            </form>
            ...
            <c:choose>
                <c:when test="${not empty discounts}">
                    <ul class="item-list">
                        ...
                        <c:forEach var="discount" items="${discounts}">
                            <c:set var="isExpired" value="${discount.endDate.isBefore(today)}"/>
                            <c:set var="notStarted" value="${discount.startDate.isAfter(today)}"/>
                            <c:set var="isActive" value="${discount.active && !isExpired && !notStarted}" />

                            <li class="item-list-item">
                                <div class="d-flex align-items-center flex-grow-1 min-width-0">
                                    <div class="item-icon">
                                        <c:choose>
                                            <c:when test="${not empty discount.product.imageUrl}">
                                                <img src="<c:url value='/${discount.product.imageUrl}'/>" alt="${discount.product.name}">
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <div class="item-details">
                                        <div class="item-title">${discount.product.name}</div>
                                        <div class="item-sub">
                                            <span class="discount-badge">-${discount.discountPercentage}%</span>
                                            <c:if test="${not empty discount.product.shop.name}">
                                                Shop: ${discount.product.shop.name}
                                            </c:if>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="text-center me-4 d-none d-md-block">
                                        <c:choose>
                                            <c:when test="${isExpired}">
                                                <span class="badge badge-status status-expired">Đã hết hạn</span>
                                            </c:when>
                                            <c:when test="${notStarted}">
                                                <span class="badge badge-status status-pending">Chưa bắt đầu</span>
                                            </c:when>
                                            <c:when test="${discount.active}">
                                                <span class="badge badge-status status-active">Hoạt động</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="badge badge-status status-inactive">Tạm dừng</span>
                                            </c:otherwise>
                                        </c:choose>
                                        <div class="text-muted small mt-1">ID: ${discount.id}</div>
                                    </div>

                                    <a href="<c:url value='/admin/product-discount/edit/${discount.id}'/>" class="action-btn action-edit" title="Sửa">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <a href="<c:url value='/admin/product-discount/toggle-status/${discount.id}'/>"
                                       class="action-btn action-toggle"
                                       title="${discount.active ? 'Tạm dừng' : 'Kích hoạt'}"
                                       onclick="return confirm('Bạn có chắc muốn ${discount.active ? 'tạm dừng' : 'kích hoạt'} giảm giá này?')">
                                        <i class="fas fa-${discount.active ? 'pause' : 'play'} "></i>
                                    </a>
                                    <a href="<c:url value='/admin/product-discount/delete/${discount.id}'/>" class="action-btn action-delete" title="Xóa" onclick="return confirm('Bạn có chắc chắn muốn xóa giảm giá này?');">
                ...
                <c:otherwise>
                     <div class="text-center p-5 border rounded bg-light">
                        ...
                        <h5 class="mb-2">Không tìm thấy giảm giá nào</h5>
                        <p class="text-muted mb-3">
                            <c:if test="${not empty keyword}">
                                Không có giảm giá nào khớp với từ khóa "<c:out value="${keyword}"/>".
                            </c:if>
                            <c:if test="${empty keyword}">
                                Hiện chưa có giảm giá sản phẩm nào trong hệ thống.
                            </c:if>
                        </p>
                        <a href="<c:url value='/admin/product-discount'/>" class="btn btn-primary">Xem tất cả giảm giá</a>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
