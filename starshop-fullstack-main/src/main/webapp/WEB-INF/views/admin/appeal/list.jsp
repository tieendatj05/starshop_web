...
<head>
    ...
    <title>Quản Lý Kháng Cáo - Admin</title>
    ...
    <style>
        ...
        .item-icon.icon-approved { background: linear-gradient(135deg, #BBF7D0, #86EFAC); color: #064E3B; }
        .item-icon.icon-rejected { background: linear-gradient(135deg, #E6EDF7, #CBD5E1); color: #111827; }
        .item-icon.icon-pending { background: linear-gradient(135deg, #FDE68A, #FBBF24); color: #78350F; }
            ...
            <c:choose>
                        ...
                        <c:forEach var="appeal" items="${appeals}">
                            <li class="item-list-item">
                                <div class="d-flex align-items-center flex-grow-1 min-width-0">
                                    <div class="item-icon 
                                        ${appeal.status == 'PENDING' ? 'icon-pending' : ''}
                                        ${appeal.status == 'APPROVED' ? 'icon-approved' : ''}
                                        ${appeal.status == 'REJECTED' ? 'icon-rejected' : ''}">
                                        <i class="fas fa-gavel"></i>
                                    </div>
                                    <div class="item-details">
                                        <div class="item-title">Kháng cáo từ: ${appeal.user.username} (${appeal.user.fullName})</div>
                                        <div class="item-sub text-truncate">Lý do: <c:out value="${appeal.reason}"/></div>
                                    </div>
                                </div>
                                    ...
                                    <div class="text-center me-4 d-none d-md-block">
                                        <c:choose>
                                            <c:when test="${appeal.status == 'PENDING'}">
                                                <span class="badge badge-status status-pending">Chờ xử lý</span>
                                            </c:when>
                                            <c:when test="${appeal.status == 'APPROVED'}">
                                                <span class="badge badge-status status-approved">Đã duyệt</span>
                                            </c:when>
                                            <c:when test="${appeal.status == 'REJECTED'}">
                                                <span class="badge badge-status status-rejected">Đã từ chối</span>
                                            </c:when>
                                        </c:choose>
                                        <div class="text-muted small mt-1">
                                            ${fn:replace(fn:substring(appeal.submittedAt, 0, 16), 'T', ' ')}
                                        </div>
                                    </div>
                                    
                                    <a href="<c:url value='/admin/appeals/view/${appeal.id}'/>" class="action-btn action-details" title="Xem chi tiết">
                ...
                <c:otherwise>
                     <div class="text-center p-5 border rounded bg-light">
                        <i class="bi bi-gavel display-4 text-muted mb-3"></i>
                        <h5 class="mb-2">Không có kháng cáo nào</h5>
                        <p class="text-muted mb-3">
                           Hiện tại không có kháng cáo nào trong hệ thống.
                        </p>
                    </div>
                </c:otherwise>
            </c:choose>
