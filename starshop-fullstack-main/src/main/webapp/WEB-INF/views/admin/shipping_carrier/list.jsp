    ...
    <div class="container">
        ...
        <div class="checkout-panel">
            <div class="section-title">
                ...
                <a href="<c:url value='/admin/shipping-carriers/add'/>" class="btn btn-themed">
                    <i class="fas fa-plus"></i> Thêm mới
                </a>
            </div>

            <%-- Search Form --%>
            <form action="<c:url value='/admin/shipping-carriers'/>" method="get" class="mb-4">
                <div class="input-group">
                    <input type="search" name="keyword" class="form-control" placeholder="Tìm kiếm theo tên, SĐT, website..." value="${keyword}" aria-label="Tìm kiếm nhà vận chuyển">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i></button>
                    <c:if test="${not empty keyword}">
                        <a href="<c:url value='/admin/shipping-carriers'/>" class="btn btn-outline-secondary"><i class="fas fa-times"></i></a>
                    </c:if>
                </div>
            </form>

            <c:choose>
                <c:when test="${not empty carriers}">
                    <ul class="item-list">
                        <c:forEach var="carrier" items="${carriers}">
                            <li class="item-list-item">
                                <div class="d-flex align-items-center flex-grow-1 min-width-0">
                                    <div class="item-icon ${carrier.isActive() ? 'icon-active' : 'icon-inactive'}">
                                        <i class="fas fa-truck"></i>
                                    </div>
                                    <div class="item-details">
                                        <div class="item-title">${carrier.name}</div>
                                        <div class="item-sub">
                                            Phí vận chuyển: <strong><fmt:formatNumber value="${carrier.shippingFee}" type="currency" currencySymbol="" maxFractionDigits="0" /> VNĐ</strong>
                                        </div>
                                        <div class="item-sub mt-1">
                                            <c:if test="${not empty carrier.website}">
                                                <i class="fas fa-globe fa-fw me-1 text-muted"></i><a href="${carrier.website}" target="_blank" class="text-decoration-none">${carrier.website}</a>
                                            </c:if>
                                            <c:if test="${not empty carrier.phone}">
                                                <i class="fas fa-phone fa-fw me-1 text-muted ${not empty carrier.website ? 'ms-3' : ''}"></i>${carrier.phone}
                                            </c:if>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex align-items-center item-actions">
                                    <div class="text-center me-4 d-none d-md-block">
                                        <c:if test="${carrier.isActive()}">
                                            <span class="badge badge-status status-active">Hoạt động</span>
                                        </c:if>
                                        <c:if test="${not carrier.isActive()}">
                                            <span class="badge badge-status status-inactive">Không hoạt động</span>
                                        </c:if>
                                        <div class="text-muted small mt-1">ID: ${carrier.id}</div>
                                    </div>
                                    
                                    <a href="<c:url value='/admin/shipping-carriers/edit/${carrier.id}'/>" class="action-btn action-edit" title="Sửa">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <a href="<c:url value='/admin/shipping-carriers/delete/${carrier.id}'/>" class="action-btn action-delete" title="Xóa" onclick="return confirm('Bạn có chắc chắn muốn xóa nhà vận chuyển này không?');">
                ...
                <c:otherwise>
                     <div class="text-center p-5 border rounded bg-light">
                        ...
                        <h5 class="mb-2">Không tìm thấy nhà vận chuyển nào</h5>
                        <p class="text-muted mb-3">
                            <c:if test="${not empty keyword}">
                                Không có nhà vận chuyển nào khớp với từ khóa "<c:out value="${keyword}"/>".
                            </c:if>
                            <c:if test="${empty keyword}">
                                Hiện chưa có nhà vận chuyển nào trong hệ thống.
                            </c:if>
                        </p>
                        <a href="<c:url value='/admin/shipping-carriers'/>" class="btn btn-primary">Xem tất cả</a>
                    </div>
                </c:otherwise>
            </c:choose>
        </div>
